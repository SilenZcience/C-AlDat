...
#ifdef BT_RECURSIVE_ADD
    return bt_add_rec(bt, key);
#elifdef BT_RECURSIVE_ADD_2
    return bt_add_rec2(bt, key);
#else
...
void _bt_print_as_dot_rec(BinTreeNode* v, FILE* f) {
    ...
    if(v->left) fprintf(f, "\t%d -> %d;\n", v->key, v->left->key);
    if(v->right) fprintf(f, "\t%d -> %d;\n", v->key, v->right->key);
...
// Gibt den kleinsten Schlüssel im Baum zurück. Der Baum muss mindestens
// einen Schlüssel enthalten.
int bt_minimum(BinTree* bt) {
    assert(bt->size > 0);

    BinTreeNode* btn = bt->root;
...
// Gibt den größten Schlüssel im Baum zurück. Der Baum muss mindestens
// einen Schlüssel enthalten.
