...
void dlist_move(DList* l, DListElement* from, DListElement* to) {
    if(from == l->head || from == l->tail) {
        return;
    }
    ...
    // haenge das element 'from' aus
    from->_next->_prev = from->_prev;
    from->_prev->_next = from->_next;

    // haenge das element hinter 'to' wieder in
    to->_next->_prev = from;
    from->_next = to->_next;
    to->_next = from;
    from->_prev = to;
...
DListElement* dlist_get(DList* l, int idx) {
    DListElement* cur=l->head->_next;
    for(int i=0; i < idx && cur != l->tail; ++i) {
        cur = cur->_next;
    }
