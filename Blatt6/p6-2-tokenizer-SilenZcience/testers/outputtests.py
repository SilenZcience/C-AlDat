...
class CompareOutputTest(StatusTest):
    ...
    def _award_points(self):
        ...
        if not self.comparer(self.std_output, self.expected_output):
            self.status = Status.WRONG_OUTPUT
            self.awarded_points = 0
        else:
            self.status = Status.SUCCESS
...
class ValgrindTest(StatusTest):
    ...
    def __init__(self, cmd, name="", description="", points=1, timeout=1, logfile="valgrind.log"):
        valgrind_cmd = f"valgrind --leak-check=full --error-exitcode=2 --log-file={logfile} {cmd} > /dev/null"
        super().__init__(
            valgrind_cmd,
            name,
            description,
    ...
    def _award_points(self):
        ...
        if self.status == Status.ERROR:
            ...
            print(f"----------------------------------------------------------------------")
            print(f"Zeilen in der Valgrind Ausgabe, die zur Bewertung führen:")
            self.awarded_points = 0
            with open(self.logfile) as valgrind_log:
                #lines = valgrind_log.readlines()
...
class TestSuite:
    ...
    def run(self):
        ...
        name_len = max([len(t.name) for t in self.test_cases])
        for t in self.test_cases:
            awarded_points += t.awarded_points
            if t.status != Status.SUCCESS:
                success = False
            print(f"  {t.name:{name_len}}   [{t.status.value:12}]  | {t.awarded_points} von {t.total_points} Punkt(en)")
